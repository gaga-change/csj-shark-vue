<template>
  <div class="BatchForm">
    ...
    <el-row v-for="item in rulesEnum" :key="item.id" style="padding:10px 30px">
      <el-radio v-model="dynamicValidateForm.addPlot" :label="item.enumValue">{{item.typeName}}</el-radio>
    </el-row>
...
import { addWaveApi, getSysEnumType } from '@/api'
export default {
  data() {
    return {
      addLotLoading: false,
  ...
  methods: {
    move(index, step) {
      ...
      console.log(this.dynamicValidateForm.ruleList)
    },
